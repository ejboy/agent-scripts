...
- Static check: run `shellcheck --severity=warning` on `scripts/mvn-lite scripts/launch-browser scripts/html-screenshot scripts/repo-map scripts/npm-lite tests/*.sh maintainers/bump-version` before finishing changes to those paths. Do not fold experimental trees under `experiments/` into repo-wide validation.
